...
// DESCRIPTION: Implements a timer suitable for use in scenarios that need to 
...
namespace LillTek.Common
{
    ...
    public sealed class PolledTimer
    {
        ...
        {
            if (interval.Ticks < 0)
                throw new ArgumentException(BadIntervalMsg, "interval");
        ...
        public void ResetRandom()
        {
            lock (syncLock)
            {
                ...
                this.fireTime = SysTime.Now + Helper.RandTimespan(interval);
                this.disabled = false;
            }
        }
        ...
        public void ResetAddRandom(TimeSpan interval)
        {
            lock (syncLock)
            {
                ...
                this.fireTime = fireTime + Helper.RandTimespan(interval);
                this.disabled = false;
            }
        }
        ...
        public void ResetRandomTemporary(TimeSpan minInterval, TimeSpan maxInterval)
        {
            if (minInterval > maxInterval)
                throw new InvalidOperationException(string.Format("PolledTimer: [minInterval={0}] is greater than [maxInterval={1}].", minInterval, maxInterval));
