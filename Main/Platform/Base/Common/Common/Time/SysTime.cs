...
// DESCRIPTION: A date/time implementation that measures time relative to 
//              the time that the system has been running rather than a
//              time based on the real-time clock.
...
// $todo(jeff.lill): 
//
// Vista and Windows Server 2008 implement GetTickCount64().  
// Come back and implement support for this at some point
// to avoid all of this wrap-around nonsense.

// $hack(jeff.lill)
//
// I've had to hack up several unit tests across the LillTek Platform
// DLLs to deal with the lack of resolution of the Envionment.TickCount
// value.  The problem is with code like:
//
//      DateTime        start,end;
//
//      start = SysTime.Now;
...
//      end = SysTime.Now;
//
// The problem is that (end-start> might be less than 1 second due to the
// fact that the TickCount may not have incremented yet.  I've corrected
// this by using SysTimer.Resolution to adjust for this.  I'm not sure
// though if there's a permanant fix though because even after I implement
// GetTickCount64() support, there will still the chance of this happening,
// albeit much less likely since the timer resolution will be much finer.
...
namespace LillTek.Common
{
    ...
    /// </para>
    /// <note>
    /// The .NET <see cref="Environment" />.<see cref="Environment.TickCount" /> property
    /// is <b>not used</b> for implementing this because its resolution will never
    /// be less than 500 milliseconds.
    /// </note>
    /// <para>
    /// The <b>GetTickCount()</b> counter is an unsigned 32-bit value and will wrap-around
    ...
    public static class SysTime
    {
        ...
        /// <summary>
        /// Initialization for Unix/Linux.
        /// </summary>
        private static void InitUnix()
        {
        ...
        public static TimeSpan Resolution
        {
            get
            {
                if (Helper.IsWindows)
                {
                    ...
                    return TimeSpan.FromMilliseconds(62);   // This is just a guesstimate based on something
                // read on the web
            }
        }
