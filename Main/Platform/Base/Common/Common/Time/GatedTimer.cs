...
// DESCRIPTION: Implements a timer that allows only one thread at a time to process timer events.
...
namespace LillTek.Common
{
    ...
    /// <remarks>
    /// <para>
    /// This is necessary to because the System.Timer class will continue firing
    /// thread handlers even if a long-running thread is still handling an earlier
    /// timer event.
    /// </para>
    /// <note>
    /// Any exceptions thrown by timer callback will be logged to the <see cref="SysLog" />.
    /// </note>
    /// </remarks>
    public sealed class GatedTimer : IDisposable
    {
        ...
        {
            // The .NET framework doesn't like really long timespans so set
            // 1 day instead.
