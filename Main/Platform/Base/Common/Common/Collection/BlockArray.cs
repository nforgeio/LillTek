...
// DESCRIPTION: Implements an array of Block instances.
...
namespace LillTek.Common
{
    ...
    /// <remarks>
    /// <para>
    /// This is the array underlying the <see cref="BlockStream" /> and 
    /// <see cref="EnhancedBlockStream" /> classes and is also used to efficiently 
    /// pass data to some LillTek.Net.Socket.EnhancedSocket methods.
    /// </para>
    /// <para>
    /// The purpose of BlockArray is to avoid performance robbing buffer 
    /// reallocations and copies as can happen with extensive use of 
    /// the MemoryStream class.  Rather than doing I/O to a single large
    /// buffer, the BlockArray class provides the underlying functionality
    /// for spreading I/O across multiple blocks.  This avoids any need
        ...
        public void CopyTo(int sourceOffset, byte[] target, int targetOffset, int length)
        {
            ...
            while (true)
            {
                ...
                else
                {
                    ...
                    lastBlockIndex++;
                    Assertion.Test(lastBlockIndex < list.Count);
