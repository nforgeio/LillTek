...
namespace LillTek.Json.Internal
{
	...
	internal class JsonReader : IDisposable
	{
		...
		{
			// parse until separator character or end
			bool end = false;
			do
			{
		...
		private bool MatchValue(string value, bool noTrailingNonSeparatorCharacters)
		{
			...
			if (!noTrailingNonSeparatorCharacters)
				return match;
			else
				return (match && (!MoveNext() || CurrentIsSeparator()));
		}

		private bool CurrentIsSeparator()
		{
			switch (_currentChar)
			{
				case '}':
