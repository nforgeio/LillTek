...
namespace LillTek.Compression.GZip 
{
	...
	/// using LillTek.Compression.GZip;
	/// 
	/// class MainClass {
	/// 
	/// 	public static void Main(string[] args) {
	/// 	
	///			using (var inStream = new GZipInputStream(File.OpenRead(args[0]))) {
    ///			
	///			    using (var outStream = File.Create(Path.GetFileNameWithoutExtension(args[0]))) {
    ///			
