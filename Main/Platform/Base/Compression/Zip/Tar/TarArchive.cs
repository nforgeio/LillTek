...
namespace LillTek.Compression.Tar
{
	...
	public class TarArchive : IDisposable
	{
		...
		/// <summary>
		/// PathPrefix is added to entry names as they are written if the value is not <c>null</c>.
					...
					while (true) {
						...
						if (asciiTrans) {
							for (int off = 0, b = 0; b < numRead; ++b) {
								if (rdbuf[b] == 10) {
									string s = Helper.ASCIIEncoding.GetString(rdbuf, off, (b - off));
									outw.WriteLine(s);
									off = b + 1;
								}
							}
					...
					using (StreamReader inStream  = File.OpenText(entryFilename)) {
						using (Stream outStream = File.Create(tempFileName)) {
							...
							while (true) {
								...
								byte[] data = Helper.ASCIIEncoding.GetBytes(line);
