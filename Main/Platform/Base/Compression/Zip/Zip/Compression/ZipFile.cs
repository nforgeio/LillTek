...
using LillTek.Compression.Encryption;
#endif

using LillTek.Common;
using LillTek.Compression.Core;
using LillTek.Compression.Checksums;
using LillTek.Compression.Zip.Compression.Streams;
using LillTek.Compression.Zip.Compression;
...
namespace LillTek.Compression.Zip 
{
	...
	/// </summary>
	/// <remarks>If the message is non-<c>null</c> an error has occured.  If the message is <c>null</c>
	...
	public class ZipFile : IEnumerable, IDisposable
	{
...
#if !COMPACT_FRAMEWORK_V10 && !MOBILE_DEVICE
		/// <summary>
		...
		/// <returns>The index position of the matching entry or <b>-1</b> if not found</returns>
		/// <exception cref="InvalidOperationException">
		/// The Zip file has been closed.
		/// </exception>
		...
		/// </param>
		/// <returns>
		/// A clone of the zip entry, or <c>null</c> if no entry with that name exists.
		/// </returns>
		/// <exception cref="InvalidOperationException">
		/// The Zip file has been closed.
		/// </exception>
		...
		/// </exception>
		/// <exception cref="LillTek.Compression.Zip.ZipException">
		...
		/// <returns><c>true</c> if all tests pass, <c>false</c> otherwise</returns>
...
*/
		...
		int WriteCentralDirectoryHeader(ZipEntry entry)
		{
			...
			byte[] rawComment = (entry.Comment != null) ? Helper.ASCIIEncoding.GetBytes(entry.Comment) : new byte[0];
...
*/
		...
		int FindExistingUpdate(ZipEntry entry)
		{
			...
			for ( int index = 0; index < updates_.Count; ++index ) {
				...
				if ( (zu.Entry.ZipFileIndex == entry.ZipFileIndex) &&
					(string.Compare(convertedName, zu.Entry.Name, CultureInfo.InvariantCulture, CompareOptions.IgnoreCase) == 0) ) {
					result = index;
					break;
				}
			}
		...
		class PartialInputStream : InflaterInputStream
		{
			...
			/// </summary>
			/// <returns>Returns the byte read or <b>-1</b> on end of stream.</returns>
	...
	class DynamicDiskDataSource : IDynamicDataSource
	{
		...
		/// <returns>Returns a stream providing data; or <c>null</c> if not available</returns>
