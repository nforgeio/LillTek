...
// DESCRIPTION: Implements the Heartbeat Service.
...
{
    /// <summary>
    /// Implements the Heartbeat Service.
    /// </summary>
    public sealed class AppService : ServiceBase, IService
    {
        ...
        private HeartbeatHandler    handler;        // The heartbeat service handler
        ...
        public string Name
        {
            get { return Const.HeartbeatName; }
        }
        ...
        public void Start(IServiceHost serviceHost, string[] args)
        {
            lock (syncLock)
            {
                ...
                NetTrace.Start();
                Program.Config = new Config("LillTek.Datacenter.HeartbeatService");
                Program.InstallPerfCounters();

                // Service initialization

                this.serviceHost = serviceHost;
                SysLog.LogInformation("Heartbeat Service v{0} Start", Helper.GetVersionString());
        ...
        public void Stop()
        {
            lock (syncLock)
            {
                ...
                SysLog.LogInformation("Heartbeat Service Stop");
                SysLog.Flush();
