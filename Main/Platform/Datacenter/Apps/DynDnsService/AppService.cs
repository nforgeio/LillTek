...
// DESCRIPTION: Implements the Dynamic DNS Service.
...
{
    /// <summary>
    /// Implements the Dynamic DNS Service.
    /// </summary>
    public sealed class AppService : ServiceBase, IService
    {
        ...
        private DynDnsHandler   handler;        // The dynamic DNS service handler
        ...
        public void Start(IServiceHost serviceHost, string[] args)
        {
            lock (syncLock)
            {
                ...
                NetTrace.Start();
                Program.Config = new Config("LillTek.Datacenter.DynDNS");
                Program.InstallPerfCounters();

                // Service initialization

                this.serviceHost = serviceHost;
                SysLog.LogInformation("Dynamic DNS Service v{0} Start", Helper.GetVersionString());
        ...
        {
            SysLog.LogInformation("Dynamic DNS Service Shutdown");
        ...
        public void Stop()
        {
            lock (syncLock)
            {
                ...
                SysLog.LogInformation("Dynamic DNS Service Stop");
                SysLog.Flush();
