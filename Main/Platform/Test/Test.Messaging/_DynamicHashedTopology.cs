//-----------------------------------------------------------------------------
// FILE:        _DynamicHashedTopology.cs
...
{
    [TestClass]
    public class _DynamicHashedTopology
    {
...
@"
clusterType = {0}:{1}
clusterArgs = cluster-ep={2}
";

        private static DynamicHashedTopology OpenClient(MsgRouter router, string clusterEP)
        {
            System.Type type = typeof(DynamicHashedTopology);

            Config.SetConfig(string.Format(ClusterConfig, type.FullName, type.Assembly.Location, clusterEP));
            return (DynamicHashedTopology)TopologyHelper.OpenClient(router, "clusterType", "clusterArgs");
        }

        private static DynamicHashedTopology OpenServer(MsgRouter router, string dynamicScope, object target, string clusterEP)
        {
            System.Type type = typeof(DynamicHashedTopology);

            Config.SetConfig(string.Format(ClusterConfig, type.FullName, type.Assembly.Location, clusterEP));
            return (DynamicHashedTopology)TopologyHelper.OpenServer(router, dynamicScope, target, "clusterType", "clusterArgs");
        }

        private class LeafRouterA : LeafRouter
        {
            private DynamicHashedTopology cluster;
            private int[] counts;
            private int index;
            ...
            public new void Start()
            {
                ...
                cluster = OpenServer(this, "A", this, "logical://A");
            }
